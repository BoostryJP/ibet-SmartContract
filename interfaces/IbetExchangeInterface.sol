...
abstract contract IbetExchangeInterface is ContractReceiver {
    ...
    /// @return 残高数量
    function balanceOf(address _account, address _token) public view virtual returns (uint256);

    /// @notice 拘束数量の参照
    /// @param _account アカウントアドレス
    /// @param _token トークンアドレス
    /// @return 拘束数量
    function commitmentOf(address _account, address _token) public view virtual returns (uint256);
    ...
    /// @param _data 任意のデータ
    function tokenFallback(address _from, uint _value, bytes memory _data) public virtual override;
