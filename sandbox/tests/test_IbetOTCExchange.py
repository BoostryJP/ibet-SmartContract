...
def transfer(token, otc_exchange, trader, amount):
    token.transfer.transact(otc_exchange.address, amount, {"from": trader})
...
def test_deploy_normal_1(
    users, otc_exchange, otc_exchange_storage, personal_info, payment_gateway
):
...
def test_deploy_error_1(users, IbetOTCExchange, personal_info, otc_exchange_storage):
    ...
    deploy_args = [1234, personal_info.address, otc_exchange_storage.address]

    with pytest.raises(ValueError):
        exchange_owner.deploy(IbetOTCExchange, *deploy_args)
...
def test_deploy_error_2(users, IbetOTCExchange, payment_gateway, otc_exchange_storage):
    exchange_owner = users["admin"]
    deploy_args = [payment_gateway.address, 1234, otc_exchange_storage.address]
    with pytest.raises(ValueError):
        exchange_owner.deploy(IbetOTCExchange, *deploy_args)
...
def test_deploy_error_3(users, IbetOTCExchange, payment_gateway, personal_info):
    exchange_owner = users["admin"]
    deploy_args = [payment_gateway.address, personal_info.address, 1234]
    with pytest.raises(ValueError):
        exchange_owner.deploy(IbetOTCExchange, *deploy_args)
...
def test_createorder_normal_1(users, otc_exchange, personal_info):
    ...
    share_token, deploy_args = utils.issue_share_token(
        users, otc_exchange.address, personal_info.address
    )
    ...
    otc_exchange.createOrder.transact(
        trader, share_token.address, _amount, _price, agent, {"from": issuer}
    )
    ...
    assert orderbook == [
        issuer.address,
        trader.address,
        to_checksum_address(share_token.address),
        _amount,
        _price,
        agent.address,
        False,
...
def test_createorder_error_1(users, otc_exchange, personal_info):
    ...
    with pytest.raises(ValueError):
        otc_exchange.createOrder.transact(
            "1234", share_token.address, _amount, _price, agent, {"from": issuer}
        )
...
def test_createorder_error_2(users, otc_exchange):
    ...
    with pytest.raises(ValueError):
        otc_exchange.createOrder.transact(
            trader, "1234", _amount, _price, agent, {"from": issuer}
        )
...
def test_createorder_error_8(users, otc_exchange, personal_info):
    ...
    _price = 123
    otc_exchange.createOrder.transact(
        trader, share_token.address, _amount + 1, _price, agent, {"from": issuer}
    )  # エラーになる
...
def test_createorder_error_9(users, otc_exchange, personal_info):
    ...
    _price = 123
    otc_exchange.createOrder.transact(
        trader, share_token.address, _amount, _price, attacker, {"from": issuer}
    )  # エラーになる
...
def test_confirmAgreement_error_7(users, otc_exchange, personal_info):
    ...
    # 決済非承認：決済業者
    otc_exchange.cancelAgreement.transact(order_id, agreement_id, {"from": _agent})
...
def test_withdrawAll_error_2_1(users, otc_exchange, personal_info):
    ...
    # 引き出し（2回目)：発行体
    otc_exchange.withdrawAll.transact(
        share_token.address, {"from": _issuer}
    )  # エラーになる
...
def test_withdrawAll_error_2_2(users, otc_exchange, personal_info):
    ...
    # 引き出し：異なるアドレス
    otc_exchange.withdrawAll.transact(
        share_token.address, {"from": _trader}
    )  # エラーになる
...
def test_updateExchange_normal_1(
    ...
    deposit_amount = 100
    share_token.transfer.transact(
        otc_exchange.address, deposit_amount, {"from": issuer}
    )
    ...
    price = 123
    otc_exchange.createOrder.transact(
        trader, share_token.address, amount_make, price, agent, {"from": issuer}
    )
    ...
    # ストレージの切り替え
    otc_exchange_storage.upgradeVersion.transact(
        otc_exchange_new.address, {"from": admin}
    )
