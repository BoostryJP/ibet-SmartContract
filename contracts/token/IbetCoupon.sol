...
contract IbetCoupon is Ownable, IbetStandardTokenInterface {
    ...
    function transferToContract(address _to, uint _value, bytes memory _data)
        ...
        returns (bool success)
    {
        require(_to == tradableExchange, ErrorCode.ERR_IbetCoupon_transferToContract_130001);
        balances[msg.sender] = balanceOf(msg.sender).sub(_value);
        balances[_to] = balanceOf(_to).add(_value);
        ...
        returns (bool success)
    {
        ...
        if (balanceOf(msg.sender) < _value) revert(ErrorCode.ERR_IbetCoupon_transfer_130101);
        if (msg.sender != tradableExchange) {
            // 譲渡可能なクーポンではない場合、エラーを返す
            require(transferable == true, ErrorCode.ERR_IbetCoupon_transfer_130102);
        }
        bytes memory empty;
        if (isContract(_to)) {
        ...
        returns (bool success)
    {
        // <CHK>
        // リスト長が等しくない場合、エラーを返す
        if (_toList.length != _valueList.length) revert(ErrorCode.ERR_IbetCoupon_bulkTransfer_130201);
        ...
        if (balanceOf(msg.sender) < totalValue) revert(ErrorCode.ERR_IbetCoupon_bulkTransfer_130202);

        // <CHK>
        // 譲渡可能ではない場合、エラーを返す
        if (msg.sender != tradableExchange) {
            require(transferable == true, ErrorCode.ERR_IbetCoupon_bulkTransfer_130203);
        }
        ...
        returns (bool)
    {
        ...
        if (balanceOf(_from) < _value) revert(ErrorCode.ERR_IbetCoupon_transferFrom_130301);
        ...
        public
    {
        ...
        if (balanceOf(msg.sender) < _value) revert(ErrorCode.ERR_IbetCoupon_consume_130401);
        ...
        public
    {
        ...
        require(initialOfferingStatus == true, ErrorCode.ERR_IbetCoupon_applyForOffering_130501);
