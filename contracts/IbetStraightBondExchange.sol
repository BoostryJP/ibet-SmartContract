...
contract IbetStraightBondExchange is Ownable {
    ...
    function createOrder(address _token, uint256 _amount, uint256 _price, bool _isBuy, address _agent) public returns (bool) {
        ...
        // 認可されたアドレスではない場合、エラーを返す
        if (WhiteList(whiteListAddress).isRegistered(msg.sender,_agent) == false) { revert(); }
        // 名簿用個人情報が登録されていない場合、エラーを返す
        if (PersonalInfo(personalInfoAddress).isRegistered(msg.sender,IbetStraightBond(_token).owner()) == false) { revert(); }
    ...
    function cancelOrder(uint256 _orderId) public returns (bool) {
        ...
        // 売り注文の場合、拘束している預かりを解放 => 残高を投資家のアカウントに戻す
        if (!order.isBuy) {
    ...
    function executeOrder(uint256 _orderId, uint256 _amount, bool _isBuy) public returns (bool) {
        ...
        // 認可されたアドレスではない場合、エラーを返す
        if (WhiteList(whiteListAddress).isRegistered(msg.sender,order.agent) == false) { revert(); }
        // 名簿用個人情報が登録されていない場合、エラーを返す
        if (PersonalInfo(personalInfoAddress).isRegistered(msg.sender,IbetStraightBond(order.token).owner()) == false) { revert(); }
        ...
        if (order.amount < _amount) {
            if (_isBuy == true) {
                ...
                // 売注文の場合、残高を投資家のアカウントに戻し、falseを返す
                balances[msg.sender][order.token] = balances[msg.sender][order.token].sub(_amount);
                IbetStraightBond(order.token).transfer(msg.sender,_amount);
                return false;
            }
        }
